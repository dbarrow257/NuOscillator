...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cerr << argv[0] << " nIterations OscillationParameterConfig [OscillatorConfig1 OscillatorConfig2 ...]" << std::endl;
    throw std::runtime_error("Invalid setup");
  }
  int nThrows = atoi(argv[1]);
  std::string OscillationParameterConfig = argv[2];
  ...
  for (size_t iOsc=0;iOsc<Oscillators.size();iOsc++) {
    ...
    for (int iThrow=0;iThrow<nThrows;iThrow++) {
      ...
      OscillationParameters_Oscillators[iOsc]["delta_cp"] = RandVal;
